...
test("3. Список отелей: frontend получает отели из PostgreSQL через GET /hotels", async ({ page }) => {
  ...
  await expect(page.locator("body")).toContainText(/найти по городу/i);
  await expect(page.locator("body")).toContainText(/найти по названию/i);

  const apiResponse = await apiGet(page, "/hotels");
