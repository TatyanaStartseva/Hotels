...
test("1. Получение авторизованным пользователем списка отелей на главной странице", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован и авторизован, в БД есть отель и комната.
...
test("2. Поиск отеля по названию среди доступных для бронирования отелей", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован и авторизован, в БД есть отель с уникальным названием.
  await createUserAndLogin(page);
...
test("3. Получение списка комнат выбранного отеля при открытии карточки отеля", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован и авторизован, в БД есть отель и связанная комната.
  await createUserAndLogin(page);
...
test("4. Создание питомца в личном кабинете с сохранением записи после перезагрузки", async ({ page }) => {
...
test("5. Серверная валидация некорректных параметров питомца при создании", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован, авторизован и находится в разделе питомцев.
  await createUserAndLogin(page);
...
test("6. Редактирование имени питомца с обновлением данных в PostgreSQL и интерфейсе", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован, авторизован и заранее создал питомца.
  await createUserAndLogin(page);
...
test("7. Удаление питомца из личного кабинета с удалением записи из backend", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован, авторизован и заранее создал питомца.
  await createUserAndLogin(page);
...
test("8. Создание бронирования комнаты и отображение брони в разделе 'Мои бронирования'", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован и авторизован, в БД есть отель и доступная комната.
...
test("9. Запрет создания бронирования с датой заезда в прошлом", async ({ page }) => {
  // Предусловие: пользователь зарегистрирован и авторизован, в БД есть отель и доступная комната.
